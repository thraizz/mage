...
func NewBlightSickle(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	...
	ability0, err := abilities.NewSpellAbilityBuilder(card.ID, card.ManaCost).
		...
		AddEffect(abilities.NewGainAbilityAttachedEffect(abilities.NewKeywordAbility(card.ID, abilities.KeywordWither), abilities.AttachmentTypeEquipment, abilities.DurationWhileOnBattlefield, "")).
		Build()
	if err != nil {
		return nil, err
	}
