...
func init() {
	cards.Register("Sentinels Mark", NewSentinelsMark)
}

// NewSentinelsMark creates a Sentinels Mark
// {1}{W} - ENCHANTMENT
// Flash
func NewSentinelsMark(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Sentinels Mark")
	card.ManaCost = "{1}{W}"
	...
	ability4, err := abilities.NewSpellAbilityBuilder(card.ID, card.ManaCost).
		AddEffect(abilities.NewGainAbilityAttachedEffect(abilities.NewKeywordAbility(card.ID, abilities.KeywordLifelink), abilities.AttachmentTypeAura, abilities.DurationUntilEndOfTurn, "")).
		Build()
	if err != nil {
		return nil, err
	}
