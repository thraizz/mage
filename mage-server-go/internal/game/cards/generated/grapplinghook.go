...
func init() {
	cards.Register("Grappling Hook", NewGrapplingHook)
}

// NewGrapplingHook creates a Grappling Hook
// {4} - ARTIFACT
func NewGrapplingHook(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Grappling Hook")
	card.ManaCost = "{4}"
