...
func NewFistsOfTheDemigod(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	...
	ability1, err := abilities.NewSpellAbilityBuilder(card.ID, card.ManaCost).
		...
		AddEffect(abilities.NewGainAbilityAttachedEffect(abilities.NewKeywordAbility(card.ID, abilities.KeywordWither), abilities.AttachmentTypeAura, abilities.DurationWhileOnBattlefield, "")).
		AddEffect(abilities.NewBoostEnchantedEffect(1, 1)).
