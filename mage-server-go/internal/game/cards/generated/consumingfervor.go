...
func init() {
	cards.Register("Consuming Fervor", NewConsumingFervor)
}

// NewConsumingFervor creates a Consuming Fervor
// {R} - ENCHANTMENT
func NewConsumingFervor(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Consuming Fervor")
	card.ManaCost = "{R}"
