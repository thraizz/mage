...
func NewTuyaBearclaw(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	...
	ability0, err := abilities.NewSpellAbilityBuilder(card.ID, card.ManaCost).
		AddEffect(abilities.NewBoostEffect(GreatestAmongPermanentsValue.POWER_OTHER_CONTROLLED_CREATURES, GreatestAmongPermanentsValue.POWER_OTHER_CONTROLLED_CREATURES)).
		Build()
	if err != nil {
		return nil, err
	}
