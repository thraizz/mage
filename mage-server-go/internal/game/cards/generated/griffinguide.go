...
func init() {
	cards.Register("Griffin Guide", NewGriffinGuide)
}

// NewGriffinGuide creates a Griffin Guide
// {2}{W} - ENCHANTMENT
func NewGriffinGuide(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Griffin Guide")
	card.ManaCost = "{2}{W}"
	...
	token3_0, err := token.GetToken("GriffinToken")
	if err != nil {
		return nil, err
	}
