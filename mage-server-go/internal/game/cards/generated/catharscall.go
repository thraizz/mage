...
func init() {
	cards.Register("Cathars Call", NewCatharsCall)
}

// NewCatharsCall creates a Cathars Call
// {2}{W} - ENCHANTMENT
func NewCatharsCall(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Cathars Call")
	card.ManaCost = "{2}{W}"
	...
	token2_0, err := token.GetToken("HumanToken")
	if err != nil {
		return nil, err
	}
