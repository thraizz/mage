...
func init() {
	cards.Register("Captains Hook", NewCaptainsHook)
}

// NewCaptainsHook creates a Captains Hook
// {3} - ARTIFACT
func NewCaptainsHook(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Captains Hook")
	card.ManaCost = "{3}"
