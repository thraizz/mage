...
func init() {
	cards.Register("Squires Devotion", NewSquiresDevotion)
}

// NewSquiresDevotion creates a Squires Devotion
// {2}{W} - ENCHANTMENT
func NewSquiresDevotion(ownerID uuid.UUID, info *cards.CardInfo) (*game.Card, error) {
	card := game.NewCard(ownerID, "Squires Devotion")
	card.ManaCost = "{2}{W}"
	...
	token3_0, err := token.GetToken("IxalanVampireToken")
	if err != nil {
		return nil, err
	}
