...
This document describes the reusable game components built for the Mage web client. These components provide a complete UI foundation for displaying and interacting with Magic: The Gathering game state.
...
<Stack
  ...
  playerNames={new Map([['player1', 'Alice'], ['player2', 'Bob']])}
...
export type CounterType =
  | 'P1P1' | 'M1M1' | 'LOYALTY' | 'POISON'
  | 'ENERGY' | 'CHARGE' | 'TIME' | 'OTHER';

export type CardZone =
  | 'HAND' | 'BATTLEFIELD' | 'GRAVEYARD'
  | 'EXILE' | 'LIBRARY' | 'STACK' | 'COMMAND';

export type GamePhase =
  | 'BEGINNING' | 'UNTAP' | 'UPKEEP' | 'DRAW'
  | 'PRECOMBAT_MAIN' | 'COMBAT' | 'DECLARE_ATTACKERS'
  | 'DECLARE_BLOCKERS' | 'COMBAT_DAMAGE' | 'END_OF_COMBAT'
  | 'POSTCOMBAT_MAIN' | 'END' | 'END_OF_TURN' | 'CLEANUP';
```
...
**Status:** All core game components are production-ready and awaiting backend game state integration.
